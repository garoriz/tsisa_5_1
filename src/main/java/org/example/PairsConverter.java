...
public class PairsConverter {
    public static void main(String[] s) {
        try {
            ...
            List<String> pairUSD_CURRENCY = Files.readAllLines(Paths.get("D:\\IntelliJ IDEA projects\\SimpleProject\\src\\main\\java\\org\\example\\USDJPY_230714_231114.csv"));
            ...
            for (int i = 1; i < Math.min(pairUSD_CURRENCY.size(), pairUSD_RUB.size()); i++) {
                ...
                pairCURRENCY_RUB.add("CADRUB;D;"+p_er[2] +";0;" + String.format("%.7f",open_ar).replaceAll(",", ".")+";"+
                        ";"+
                        ";"+
                        String.format("%.7f",close_ar).replaceAll(",", ".")+";");

                Files.write(Paths.get("JPYRUB_230714_231114.csv"),pairCURRENCY_RUB);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
